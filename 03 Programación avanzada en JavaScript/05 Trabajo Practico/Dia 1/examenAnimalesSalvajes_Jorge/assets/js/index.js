...
//Recuperación y formateo de datos JSON + Carga.
(async () => {
	const url = "animales.json";
	try {
		const res = await fetch(url);
		const data = await res.json();
		registrarAnimalyCrearInstancia(data);
		getAnimalImagePrevisualizacion(data);
	} catch (error) {
		console.log(error);
	} finally {
	}
...
// Recuperar la imagen de vista previa del animal seleccionado una vez que se selecciona de la lista desplegable.
const getAnimalImagePrevisualizacion = (data) => {
...
// Registrar animal una vez que se presiona el botón "Agregar".
const registrarAnimalyCrearInstancia = (data) => {
	const btnRegistrar = document.getElementById("btnRegistrar");

	let animalArray = [];
	btnRegistrar.addEventListener("click", (e) => {e.preventDefault();
		...
		try {
			...
			// Realizar validaciones antes de permitir que el usuario registre un animal.
...
// Dibujar el animal en la tabla "Animales en Investigacion".
...
// Funciones de audio de tarjetas de animales - Totalmente funcional.
...
// Llamado modal - Totalmente funcional.
...
// Función crear modales animales.
