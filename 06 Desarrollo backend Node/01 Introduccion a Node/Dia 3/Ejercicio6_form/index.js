...
const server = http.createServer((request, response) => {
    ...
    if (request.url.startsWith('/crear?name=')) {
        ...
        fs.writeFile(`Ejercicio6_form/files/${name}`, message, 'utf8', (err) => {
            if (err) {
        ...
        const { name } = params
        fs.readFile(`Ejercicio6_form/files/${name}`, 'utf8', (err, data) => {
            if(err){
        ...
        fs.rename(`Ejercicio6_form/files/${name}`,`Ejercicio6_form/files/${newName}`,(err)=>{
            if(err){
        ...
        const { name } = params
        fs.unlink(`Ejercicio6_form/files/${name}`, (err)=>{
            if(err){
                response.write('Ha ocurrido un error al eliminar el archivo...')
                return response.end()
            }
            response.write('Se ha eliminado el archivo exitosamente...')
            response.end()
        })
    }
    else {
